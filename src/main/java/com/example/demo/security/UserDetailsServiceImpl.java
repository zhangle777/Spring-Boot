...
//    return user;
    logger.info("用户的用户名: {}", userName); // TODO 根据用户名，查找到对应的密码，与权限 // 封装用户信息，并返回。参数分别是：用户名，密码，用户权限
    org.springframework.security.core.userdetails.User user = new User(userName, "123456", AuthorityUtils.commaSeparatedStringToAuthorityList("admin"));
    return user;
  }
}
